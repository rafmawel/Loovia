...
export function MockupDashboard({ className = '' }: { className?: string }) {
  return (
    <div className={`bg-bg-card rounded-2xl border border-border shadow-[0_40px_80px_rgba(0,0,0,0.5)] overflow-hidden ${className}`}
    ...
    >
      ...
      <div className="flex">
        {/* Sidebar mock */}
        <div className="w-44 border-r border-border p-3 hidden sm:block">
          <div className="flex items-center gap-2 mb-6 px-2">
            <div className="h-5 w-5 rounded bg-accent/20" />
            <span className="text-xs font-bold text-accent font-display">Loovia</span>
          </div>
          {['Tableau de bord', 'Mes Biens', 'Locataires', 'Baux', 'Finances', 'Analytique'].map((item, i) => (
            <div key={item} className={`flex items-center gap-2 px-2 py-1.5 rounded-md text-[11px] mb-0.5 ${i === 0 ? 'bg-accent/10 text-accent font-medium' : 'text-text-secondary'}`}>
              <div className={`h-3 w-3 rounded ${i === 0 ? 'bg-accent/30' : 'bg-border'}`} />
              {item}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 p-4 sm:p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
            {[
              ...
              <div key={kpi.label} className="bg-bg-primary rounded-lg p-2.5 border border-border">
          ...
          {/* Chart area */}
          <div className="bg-bg-primary rounded-lg border border-border p-3 mb-3">
          ...
          <div className="bg-bg-primary rounded-lg border border-border p-3">
            ...
            {[
              ...
              <div key={item.text} className="flex items-center justify-between py-1.5 border-b border-border last:border-0">
